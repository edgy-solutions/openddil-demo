  ...
  return (
    <header className="panel flex items-center justify-between p-3 m-2 shrink-0 z-10 border-b-2 border-b-slate-700 transition-colors duration-500">
      <div className="flex items-center space-x-6 w-full max-w-6xl mx-auto">
        ...
        <div className="flex-1 flex flex-col items-center relative">
          <div className={`absolute w-full h-[2px] top-3 -z-10 ${wanActive ? 'bg-slate-700' : 'bg-rose-900'}`}></div>
          ...
          <span className={`text-[10px] mt-2 font-bold tracking-widest ${wanActive ? 'text-emerald-400' : 'text-rose-500 glow-rose'}`}>
            {wanActive ? 'HQ WAN: ACTIVE' : 'HQ WAN: SEVERED'}
          </span>
        </div>
